...
int r5_cpa_pke_encrypt(uint8_t *ct, const uint8_t *pk, const uint8_t *m, const uint8_t *rho) {
    ...
    error_blocking=(1<<(PARAMS_T_BITS - PARAMS_B_BITS-1));     // error blocking bit for the EMBLEM encoding
...
#endif
        t = (modp_t) (t + ((tm & ((1 << PARAMS_B_BITS) - 1)) << (PARAMS_T_BITS - PARAMS_B_BITS)) + error_blocking) & ((1 << PARAMS_T_BITS) - 1);
...
int r5_cpa_pke_decrypt(uint8_t *m, const uint8_t *sk, const uint8_t *ct) {
    ...
    for (i = 0; i < PARAMS_MU; i++) {
        ...
        x_p = (modp_t) ((x_p >> (PARAMS_P_BITS - PARAMS_B_BITS)) & ((1 << PARAMS_B_BITS) - 1));   // changed into EMBLEM decoding implementaion
